...
export function updateSelectedRows(state: State): State {
    ...
    const updatedRows = state.cellMatrix.rows.filter(r => state.selectedRowIds.includes(r.id)).sort((a, b) => a.idx - b.idx);
    ...
    return {
        ...
        selectedRowIds: updatedRows.map(row => row.id)
    }
}
...
export function updateSelectedColumns(state: State): State {
    ...
    // TODO this filter is very inefficient for big tables
    const updatedColumns = state.cellMatrix.cols.filter(r => state.selectedColIds.includes(r.id)).sort((a, b) => a.idx - b.idx);
    ...
    return {
        ...
        selectedColIds: updatedColumns.map(col => col.id)
    }
}
